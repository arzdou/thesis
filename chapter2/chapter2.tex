...
At the center of the SMPD lies a superconducting transmon qubit\sidecite{koch_chargeinsensitive_2007}. This element behaves as a non-linear oscillator and can be treated as an effective two-level-system with frequency $\omega_q$ and levels $\ket{g}$ and $\ket{e}$. The qubit is simultaneously coupled to two independent resonators: the \textit{buffer} resonator with frequency $\omega_b$ and the \textit{waste} resonator with frequency $\omega_w$. The process that enables the frequency conversion is a parametric four-wave mixing (4WM), in which an \textit{buffer} photon is mixed with a coherent pump tone to produce an excitation on the qubit and a \textit{waste} photon.

The impinging photon is first captured by the \textit{buffer} resonator and waits for the 4WM process to begin, hence its name. The process is parametrically controlled with a coherent pump tone with frequency $\omega_p$. For the 4WM process to take place, the following energy conservation relation must be followed

\begin{equation}
    \omega_b + \omega_p = \omega_q + \omega_w,
\end{equation}

which can be ensured by modifying the frequency of the pump tone. When this condition is met, the \textit{buffer} photon is coherently transformed into a qubit excitation and a photon on the \textit{waste} resonator. To avoid the inverse process, a dissipation channel on the \textit{waste} resonator is engineered by strongly coupling it to the microwave lines. Consequently the \textit{waste} photon is rapidly lost, blocking the inverse process from ever taking place. Finally the state of the qubit is reset and the process can start again. This allows the detection of any incoming photons during the detection window, which is defined by the duration of the pump tone, in a cyclic manner. \red{reffig} shows a schematic view of the process. 
