...
where $\sigma^{(\Downarrow)}_n=1$~Hz is the measurement uncertainty. The Hamiltonian fit was performed via Monte-Carlo Markov-Chain simulation \sidecite{foreman-mackey_emcee_2013}. In brief, the algorithm initializes an arbitrary number of walkers, or particles, in the parameter space. After an evaluation of the logarithm of the likelihood function, the positions of the walkers are updated through the "stretch move" \sidecite{goodman_ensemble_2010} with the objective of minimizing the evaluation for the next iteration. We run the algorithm with 64 walkers and 60000 iterations, from which we obtain the following median values for the ground and excited state parameters
...
To further test our model, we take a step back in the approximations and fit the complete frequency dataset. In addition, we now introduce a term that models the electron spin dependent nuclear quadrupole measured in the previous section. We model this term in analogy to a standard quadrupole term, coupled to the state of the electron spin. However, given that our measurements are maximally sensitive along the $z$ axis we only consider first order effects,

\begin{equation}\label{eq:SDQ}
\mathcal{H}_\mathrm{sdq} = \frac{C_\mathrm{sdq}}{4 I (2I-1)} S_z \left(3 I_z^2 - I(I+1) \right).
\end{equation}

Where $C_\mathrm{sdq}$ is the interaction coupling strength. Moreover, to account for the high spectral resolution of our data we must include in the Hamiltonian higher order terms we have previously disregarded. 

\subsubsection{Lamb shift}

First, we consider the Lamb shift of the electron spin transitions, which renormalize the \Nb transitions in the excited state of the \Er spin. As discussed in \refsec{purcell_and_lamb}, the Lamb shift of the excited state of a spin-1/2 coupled to a resonator in its (vacuum) ground state, with coupling strength $g_0$ and detuning $\Delta$ is given by
...
Second we consider non-secular hyperfine terms, which are estimated by magnetic dipole interaction simulations (see \refsec{interaction_between_to_spins}) and are fixed during the fitting procedure. Additionally, we fix the perpendicular hyperfine term to the measured value in \refsec{solid_effect_ninehalf} $A_\perp = 46$~kHz to effectively reduce the number of variables to fit.
...
\begin{split}
    ...
    S_2/2\pi = -149.457(5)\text{~kHz}\; &| \;  C_\mathrm{sdq}/2\pi= 5.4(4) \text{~kHz}\\
...
\begin{marginfigure}
    ...
    \caption[SDQ coupling strength vs in-plane angle]{Spin dependent quadrupole term $Q_{zzz}^\text{(SDQ)}$ obtained by fitting the \Nb nuclear spin transitions as a function of the in-plane angle $\theta$}
    \labfig{sdq_vs_angle}
\end{marginfigure}

The quadrupolar tensor $\bar{\bar{Q}}$ is graphically represented in the crystal frame in \reffig{quadrupole3d}. We compare the fitted quadrupole tensor with DFT calculations performed by our theory collaborator Thibault Charpentier. The simulations yield $C_{q,\text{DFT}}=22.7$~MHz, $\eta_{\text{DFT}}=0.76$, and similar orientation of the principal axes, in close agreement with the fitted values. This confirms that $\mathrm{Nb}^{5+}$ replaces $\mathrm{W}^{6+}$. Finally, the SDQ term $C_\mathrm{sdq}/2\pi= 5.4(4)$~kHz is fitted to a non-zero value with a high degree of confidence. We check the consistency and reproducibility of our analysis by repeating these measurements for a small range of in-plane angles $\theta$ (see \reffig{sdq_vs_angle}). Despite the vastly different values of the Hamiltonian parameters (in particular, $\omega_I$ and $A$), the values extracted for $C_\mathrm{sdq}$ remain approximately unchanged. 

\section{Origin of the SDQ coupling}
...
First is the pseudo-Zeeman effect \sidecite{baker_paramagnetic_1997, jeschke_hyperfine_1997}. The magnetic moment of the $Z_1$ doublet states is modified which can result in non-zero average magnetic moment. This generates an effective magnetic field at the location of the nuclear spins, which effectively renormalizes the gyromagnetic factor of the nuclear spin. 
...
The hyperfine coupling tensor $\bar{\bar{A}}$ is calculated from a dipole-dipole interaction between the \Er spin and the \Nb at the expected position. The crystal-field parameters are obtained from ref.\sidecite{enrique_optical_1971}. The rest of the values are taken from the spectroscopic measurements performed in \refsec{final_hamiltonian_fit}. Given that the pseudo-quadrupole effect is a second order interaction of the hyperfine coupling, we use the scaling parameter $\lambda$ to control the strength the interaction. 
...
\begin{figure}
    ...
    \textit{(left)} The difference between the quadrupole principal values $Q_i^{(\Uparrow)} - Q_i^{(\Downarrow)}$ is plot as a function of the hyperfine scaling parameter $\lambda$. The vertical line marks $\lambda=0$. The simulation was performed for $\theta=-0.6^\circ$ and $\beta_0=-0.57^\circ $.
    \textit{(right)} $Q_i^{(\Uparrow)} - Q_i^{(\Downarrow)}$ as a function of $\theta$, the angle between the magnetic field and the projection of the $c$-axis in the resonator plane, with $\lambda=1$ and $\beta_0=-0.57^\circ$.}
    \labfig{pseudoquadrupole}
\end{figure}

The simulated NMR transitions are then fit with the effective spin-1/2 model. The fit is performed following the procedure outlined in the previous sections, by separating the fit between the electron ground and excited state and compares the principal values of the quadrupole in the two manifolds. Figure \reffig{pseudoquadrupole} plots the difference between the quadrupole principal values $Q_i^{(\Uparrow)} - Q_i^{(\Downarrow)}$ as a function of the scaling parameter $\lambda$.

The pseudo-quadrupole shift is non-zero and quadratic with the hyperfine scaling factor, in agreement with a second-order effect. The interaction matches qualitatively with the experimentally observed mismatch splitting of the quadrupole parameters. However, the predicted strength of the interaction along the $c$-axis ($i=Z$) for $\lambda=1$ is approximately $-8$~Hz, an order of magnitude smaller compared to the measurement and with opposite sign. We continue this analysis by fixing $\lambda=1$ and scanning the misalignment angle $\theta$ between $B_0$ and the projection of the $c$-axis on the resonator plane. \red{make in the same order as $C_{sdq}$} In the span of $\pm5$ degrees the shift remains consistently smaller than 10~Hz (in absolute value) with no indication of a rapid increase.

From this numerical analysis, we conclude that the effect of the pseudo-quadrupole interaction has a magnitude comparable with the data but opposite sign, indicating that this mechanism should be considered but cannot be the only one at play. 

\subsection{Magnetically induced electric dipole}

In \labsec{induced_dipole} we introduced the concept of magnetically induced electric dipole. Indeed, ions in non-centrosymmetric sites will experience admixing of wavefunctions with different parity due to the odd terms of the crystal field Hamiltonian \sidecite{kiel_theory_1966}. In turn, this results in a magnetoelectric effect which couples static electric and magnetic fields and can be effectively described as a magnetically induced electric dipole. As described in \labsec{induced_dipole}, the Hamiltonian used to describe this effect is 
