...
The hyperfine coupling tensor $\bar{\bar{A}}$ is calculated from a dipole-dipole interaction between the \Er spin and the \Nb at the expected position. The crystal-field parameters are obtained from ref.\sidecite{enrique_optical_1971}. The rest of the values are taken from the spectroscopic measurements performed in \red{ref}. Given that the pseudo-quadrupole effect is a second order interaction of the hyperfine coupling, we use the scaling parameter $\lambda$ to control the strength the interaction. 
