...
This chapter is dedicated to the description of the \Er electron-spin and the nuclear-spins in \Ca along with their interactions. The chapter will be organised as follows. \refsec{erbium_ions_in_ca} begins with the description of the energy levels of a Rare-Earth Ions (REIs) impurity in a crystal to arrive to an effective electron-spin-1/2 approximation. This is followed in \refsec{nuclear_spins_ca} by a description of the nuclear-spin Hamiltonian including the quadrupole interaction. Finally, the hyperfine interaction between an electron and a nuclear spin is discussed in \refsec{interaction_between_to_spins}.

\section{\Er ions in \Ca}
\labsec{erbium_ions_in_ca}

Erbium belongs to the family of lanthanides, which in solids become a trivalent ion whose common property is a partially filled $4f$ shell. With eleven electrons in the $4f$ manifold, an odd number, the ion possesses an unpaired electron, which is the origin of its paramagnetic properties. The common property for lanthanides is the shielding of the $4f$ shell by the more external electrons of the $5s$ and $5p$ closed shells. This protection reduces the interaction with ligand electrons and phonons in solids. This allows one to treat the electrostatics of the crystal as a perturbation to the Free-Ion Hamiltonian. 

In this section we sketch a derivation of the Free-Ion Hamiltonian of \Er, based on \sidecite{weissbluth_atoms_2012, abragam_electron_2012}. From the multi-electron Hamiltonian we arrive to an electronic system of degenerate manifolds of well-defined total angular momentum $J$ with a $16$-fold-degenerate ground state manifold with $J=15/2$.
...
We can estimate the average distance between \Er ions by considering the ion's density in crystal of 3.1~ppb (see \refsec{sample_details}). There are four calcium sites in a single unit cell with volume $V_{\text{unit cell}}\sim310\ \text{\AA}^3$. This results in in average distance between ions of 0.3 \textmu m. \sidenote{The Ca site density is 
\begin{equation*}
...
\section{Electron-nucleus interaction}
\labsec{interaction_between_to_spins}

In this thesis, individual \Er ions in \Ca are used as ancilla spins to address individual nuclear-spins in their vecinity. We now proceed to consider the interaction between the two spin species which is mainly dominated via the long-range dipolar interaction\sidenote{
    We do not consider exchange interaction between the electron and nuclear-spins. The \Er and W ions in the crystal are coordinated by an oxygen ion (see \labfig{crystal_structure}) which prevents direct overlap between the spin wavefunctions. 
}.
