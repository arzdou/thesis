...
\red{Plot with the shift as a function of the electric field}
...
\begin{equation}
\mathcal{H}_{Q} = \frac{eQ}{6I(2I-1)} 
\sum_{\alpha,\beta} V_{\alpha\beta} 
\left[ \tfrac{3}{2} \left( I_{\alpha} I_{\beta} + I_{\beta} I_{\alpha} \right) 
- \delta_{\alpha\beta} I^{2} \right].
\end{equation}

where $Q$ denotes the quadrupole moment obtained from the integral of the nuclear charge distribution. We note the following relations between the values of the EFG. The Laplace law enforces that $\nabla^2 V = V_{xx} + V_{yy} + V_{zz} = 0$. In addition, the symmetry of the EFG results in $V_{\alpha\beta} = V_{\beta\alpha}$. Therefore, the EFG can be effectively described as a real, symmetric and traceless rank-2 tensor which can be diagonalized. In the principal-axis system $(\mathbf{x}_Q, \mathbf{y}_Q, \mathbf{z}_Q)$ and choosing the $\mathbf{z}_Q$-axis such that $|V_{z_Qz_Q}|\geq|V_{x_Qx_Q}|\geq|V_{y_Qy_Q}|$, it is customary to introduce the asymmetry parameter
...
\begin{equation}
\mathcal{H}_{Q} = \frac{\hbar \omega_Q}{6}\left[ 3 I_{z_Q}^2 - I(I+1) + \eta(I_{x_Q}^2 - I_{y_Q}^2)\right],
\end{equation}

where $\hbar \omega_Q$ is the quadrupole energy\sidenote{\[\omega_Q \equiv  \frac{3C_q}{2I(2I-1)\hbar} \]}.
