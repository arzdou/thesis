...
\noindent where $\alpha = |\alpha| e^{i\phi}$ and $\Omega_d = 2g_0|\alpha|$. This corresponds to the classical two-level-system under drive and has an analytical solution. The dynamics of this effective two-level system under drive are exactly solvable.  If the spin is initially in the ground state, the excited-state population at time $t$ is
