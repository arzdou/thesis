...
$T_{c,b,a}$    & 2.9    &  (-) 
\end{tabular}
\caption{Magneto-electric parameters for \Er:\Ca given in ($10^{-32} \times \frac{\text{J/T}}{\text{V/m}}$). The third column indicates the relation between the parameters that originate from the $S_4$ symmetry of the ion site. The rest of the values of the tensor can be obtained by enforcing $\{i, j\}$ index symmetry $T_{k, i, j} = T_{k, j, i}$.}
\labtab{magnetoelectric_parameters}
\end{margintable}

where $i,j,k \in\{x,y,z\}$ and $T_{k, i, j}$ is third-rank tensor which is symmetric in the {i, j} indices. The tensor captures the linear mixing of the levels as a function of the electric and magnetic field. The values for the tensor in \Er:\Ca were experimentally determined by Mims \sidecite{mims_electric_1965} and are displayed in \reftab{magnetoelectric_parameters}. The effect can be discribed with only four independent parameters and the rest of the elements of the tensor are obtained by enforcing $S_4$ and $\{i, j\}$ index symmetry.
...
This distance is large enough that interactions between paramagnetic impurities can be effectively neglected except in specific cases where two ions are, by chance, in close proximity. Consequently, we consider that the magnetic noise environment in the crystal is dominated by \W nuclear spin.
...
In classical terms, the magnetic interaction between two dipoles with magnetic moment $\boldsymbol{\mu_1}$ and $\boldsymbol{\mu_2}$ is expressed as\sidecite{abragam_electron_2012}
...
\noindent where $\mathbf{r}$ is the vector separating the two 

\begin{equation}
\begin{aligned}
\mathcal{H}_{dd} &= 
\mu_B^2 \frac{\mu_0}{4\pi} r^{-3} \Big[ 
g_{x1} g_{x2} \hat{S}_{x1} \hat{S}_{x2} \big( 1 - 3 \sin^2 \theta_r \cos^2 \phi_r \big) \\
&\quad + g_{y1} g_{y2} \hat{S}_{y1} \hat{S}_{y2} \big( 1 - 3 \sin^2 \theta_r \sin^2 \phi_r \big) \\
&\quad + g_{z1} g_{z2} \hat{S}_{z1} \hat{S}_{z2} \big( 1 - 3 \cos^2 \theta_r \big) \\
&\quad - 3 \sin^2 \theta_r \cos \phi_r \sin \phi_r 
\big( g_{x1} g_{y2} \hat{S}_{x1} \hat{S}_{y2} + g_{y1} g_{x2} \hat{S}_{y1} \hat{S}_{x2} \big) \\
&\quad - 3 \sin \theta_r \cos \phi_r \cos \theta_r 
\big( g_{x1} g_{z2} \hat{S}_{x1} \hat{S}_{z2} + g_{z1} g_{x2} \hat{S}_{z1} \hat{S}_{x2} \big) \\
&\quad - 3 \sin \theta_r \sin \phi_r \cos \theta_r 
\big( g_{y1} g_{z2} \hat{S}_{y1} \hat{S}_{z2} + g_{z1} g_{y2} \hat{S}_{z1} \hat{S}_{y2} \big) 
\Big]
\end{aligned}
\end{equation}

The exchange interaction\sidenote{Also known as Fermi-contact interaction} takes the following form \sidecite{abragam_electron_2012}:

\begin{equation}
    \cH{exch} = \mathbf{S_1} \cdot \bar{\bar {J}}_\text{exch} \cdot \mathbf{J}
\end{equation}
...
In the case of electron and nuclear spin interaction, the \Er and W ions in the crystal are coordinated by an oxygen ion (see \red{labfig}) which prevents direct overlap between the spin wavefunctions. We conclude that contact interaction between proximal \Er ions and \W nuclear spins is not the primary coupling; at most they may produce small isotropic shifts.

As such the dominating interaction mechanism between two spins in our system is the free-space dipolar coupling
